...
if(isset($_SERVER['REQUEST_METHOD'])) {
    ...
    for($i = 0; $i < count($inputData); $i++) {
        $like_check_query = "SELECT COUNT(is_like) AS `like_cou` FROM `sub_question_and_answers` WHERE `ques_id` = $inputData[$i] AND `is_like` = 1;";
    ...
    for($i = 0; $i < count($inputData); $i++) {
        $dislike_check_query = "SELECT COUNT(is_dislike) AS `dislike_cou` FROM `sub_question_and_answers` WHERE `ques_id` = $inputData[$i] AND `is_dislike` = 1;";
