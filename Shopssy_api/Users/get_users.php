...
$table_of_user_data_query = "SELECT register.user_id, register.f_name, register.l_name, register.email, register.status AS `r_status`, account.my_name, account.street, account.city, account.state, account.zip, account.phone, account.country, account.status AS `a_status` FROM `register` INNER JOIN `account` ON register.user_id = account.user_id";
