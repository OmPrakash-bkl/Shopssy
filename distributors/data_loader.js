...
function reloader() {
    let ordered_datas = order_details_retriever("POST", "../ordered_user_details/details_of_ordered_user_2/", "");
...
function show_ordered_user_details(searchData) {
    ...
    function UI_Fun_1(datas) {
        ...
        lastchar = qty.substr(qty.length - 1);
            ...
            <td><button title="Edit"  onclick="makeOrderProcess(${resultData[0][i].order_id}, ${resultData[0][i].user_id})" class="edit_button_of_table">Ready</button></td>
            </tr>`
         
        }
        document.getElementsByClassName("shopssy_distributors_details_table")[0].innerHTML = table_datas;
    }
    
    if(searchData == '') {
        let responseObj = order_details_retriever("POST", "../ordered_user_details/details_of_ordered_user_2/", "");
        display_preLoader();
...
function makeOrderProcess(o_id, user_id) {
    display_preLoader();
    let datas = {
        order_id: o_id,
        u_id: user_id
    ...
    let response = order_details_retriever("POST", "../ordered_user_details/made_prod_ready/", `${datas}`);
    ...
    if(!(searchWords == "")) {
        ...
        searchWords = {
           ...
           searchWords = JSON.stringify(searchWords);
            ...
            let responseObjs = order_details_retriever("POST", "../ordered_user_details/search_details_2/", `${searchWords}`);
            display_preLoader();
            ...
            responseObjs.then((response) => {
                ...
                console.log(error);
                document.getElementsByClassName("shopssy_distributors_details_table")[0].innerHTML  =  `<center>
                <h2>No Results</h2>
                </center>`
            })
